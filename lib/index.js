...
(function (global) {
    ...
    if (Object.prototype.toString.call(args) === '[object Array]') {
      ...
      for (var item in args) {
        ...
        if (global.GMP.maps[that.id].markers[uid]) {
          ...
          marker = global.GMP.maps[that.id].markers[uid];

          // Looping only when necessary
          if (s.count === 1) {
            _updateMarker(marker,s.setterKey, s.setters[s.setterKey]);
          } else {
            for (var setterKey in s.setters) {
              _updateMarker(marker, setterKey, s.setters[s.setterKey]);
            }
          }
        }
      }
  ...
  function _updateMarker(marker, key, value)
  {
    if (key == 'setMove') {
  ...
  GMP.prototype.updateGroup = function(name, options) {
    ...
    if (global.GMP.maps[that.id].groups && global.GMP.maps[that.id].groups[name]) {
      ...
      for (var item in global.GMP.maps[that.id].groups[name]) {
        ...
        // Looping only when necessary
        if (s.count === 1) {
          marker[s.setterKey](s.setters[s.setterKey]);
        } else {
          for (var setterKey in s.setters) {
            marker[setterKey](s.setters[setterKey]);
          }
        }
      }
    }
