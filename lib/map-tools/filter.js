...
module.exports = function(global, that, type) {
  ...
  function filterByTag(query) {
    ...
    if (typeof query === "string"){
      ...
      tags = query;
      var i, uid, tag, collection, firstCollection = {};
      ...
      for (i in tags) {
        ...
        collection = that[type].tags[tag];

        if (count == 1) {

          // Clone first collection
          for (var uid2 in collection) {


            firstCollection[uid2] = collection[uid2];
          ...
          for (uid in collection) {
            ...
            if (firstCollection[uid]) {
              ...
              console.log('delete me ------------- ',collection[uid].title, tag);

              delete firstCollection[uid]
            }
          }
        }
      }

      return utils.toArray(firstCollection);
      ...
      function fetchByTag(query){
        for (var i in query) {
          var tag = query[i];
          var res= that[type].tags[tag]

          try{
            var nextTag = query[i+1];
            res = getCommonObject(res,that[type].tags[nextTag])
          } catch(e){
            // end of the array
          }
