  ...
  GMP.prototype.updateMarker = function(args, options) {
    ...
    if (Object.prototype.toString.call(args) === '[object Array]') {
      ...
      for (var x in args) {
        ...
        /* If the 1st parameter is an instance of a Marker, update directly */
        if (item instanceof global.google.maps.Marker) {
          _updateMarker(item, s);
        } else {
          if (item.uid && GMP.maps[that.id].markers[item.uid]) {
            _updateMarker(GMP.maps[that.id].markers[item.uid], s);
          }
        }
      }
    }
