  ...
  function load(options, cb) {
    ...
    if (validOptions(options, cb)) {
      ...
      if (options.filters) {
        ...
        global.GMP.maps[id].markers.filter = global.GMP.maps[id].markers.filters || {};
        var filter, dimension;
        for (dimension in options.filters) {
          filter = global.GMP.maps[id].markers.crossfilter.dimension(options.filters[dimension]);
          global.GMP.maps[id].markers.filter[dimension] = filter;
        }
      }
